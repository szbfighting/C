...
int Ticket_Srv_GenBatch(int schedule_id) {
	...
	seat_list_t seat_head, pos;
	List_Init(seat_head, seat_node_t);
	int count = Seat_Srv_FetchValidByRoomID(seat_head, buf.studio_id);

	ticket_list_t ticket_head;
	List_Init(ticket_head, ticket_node_t);
	
	play_t play_node;
	Play_Srv_FetchByID(buf.id, &play_node);

	int key = EntKey_Perst_GetNewKeys("ticket", buf.seat_count);

	List_ForEach(seat_head, pos) {
		ticket_list_t node;
		node = (ticket_list_t)malloc(sizeof(ticket_node_t));
		node->data.id = key++;
		node->data.price = play_node.price;
		node->data.schedule_id = buf.id;
		node->data.seat_id = pos->data.id;
		node->data.status = TICKET_AVL;
		List_AddTail(ticket_head, node);
	}
	if (Seat_Srv_FetchValidByRoomID(seat_head, buf.studio_id)) {
